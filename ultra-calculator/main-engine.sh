#!/bin/bash
# ULTRA CALCULATOR v5 - UC Title + Way + Bilingual
...
echo -e "${D}  [+ tambah/add] [- kurang/sub] [* kali/mul] [: bagi/div] [/ per]${END}\n"
}
...
${Y}${BOLD}ALJABAR:${END}
  ${G}2x+3x${END} -> 5x
  ${G}x=5${END} -> simpan
  ${G}way 10:2+5*2${END} -> lihat cara
  ${G}way 2x+3=11${END} -> cara aljabar
${Y}${BOLD}PERINTAH:${END}
  help, vars, history, clear, exit, way
" ;;
...
way_numeric(){
  ...
  for _ in $(seq 1 30); do
    [[ "$cur" =~ ^-?[0-9.]+$ ]] && break
    if [[ "$cur" =~ sqrt\([^()]+\) ]]; then
      match=$(echo "$cur" | grep -oE 'sqrt\([^()]+\)' | head -n1); inner=$(echo "$match" | sed -E 's/sqrt\((.*)\)/\1/'); val=$(echo "scale=10; sqrt($inner)" | bc -l 2>/dev/null); [[ -z "$val" ]] && break
      val_f=$(echo "$val" | awk '{printf "%g", $1}'); echo -e "${G}$(t step) $step:${END} $match = $val_f"
      cur=$(awk -v c="$cur" -v o="$match" -v n="$val_f" 'BEGIN{pos=index(c,o); print substr(c,1,pos-1) n substr(c,pos+length(o))}'); echo -e "  -> $cur"; step=$((step+1)); continue
    fi
    if [[ "$cur" =~ \([^()]+\) ]]; then
      match=$(echo "$cur" | grep -oE '\([^()]+\)' | head -n1); inner=$(echo "$match" | tr -d '()'); val=$(echo "scale=10; $inner" | bc -l 2>/dev/null); [[ -z "$val" ]] && break
      val_f=$(echo "$val" | awk '{printf "%g", $1}'); echo -e "${G}$(t step) $step:${END} $match = $val_f"
      cur=$(awk -v c="$cur" -v o="$match" -v n="$val_f" 'BEGIN{pos=index(c,o); print substr(c,1,pos-1) n substr(c,pos+length(o))}'); echo -e "  -> $cur"; step=$((step+1)); continue
    fi
    if echo "$cur" | grep -qE '[0-9.]+\^[0-9.]+'; then
      match=$(echo "$cur" | grep -oE '[0-9.]+\^[0-9.]+' | head -n1); l=$(echo "$match" | cut -d'^' -f1); r=$(echo "$match" | cut -d'^' -f2); val=$(echo "scale=10; $l ^ $r" | bc -l); val_f=$(echo "$val" | awk '{printf "%g", $1}')
      echo -e "${G}$(t step) $step:${END} $l ^ $r = $val_f"; cur=$(awk -v c="$cur" -v o="$match" -v n="$val_f" 'BEGIN{pos=index(c,o); print substr(c,1,pos-1) n substr(c,pos+length(o))}'); echo -e "  -> $cur"; step=$((step+1)); continue
    fi
    if echo "$cur" | grep -qE '[0-9.]+[*\/][0-9.]+'; then
      match=$(echo "$cur" | grep -oE '[0-9.]+\s*[*\/]\s*[0-9.]+' | head -n1); clean=$(echo "$match" | tr -d ' '); op=$(echo "$clean" | grep -oE '[*\/]'); l=$(echo "$clean" | awk -F'[*\/]' '{print $1}'); r=$(echo "$clean" | awk -F'[*\/]' '{print $2}'); val=$(echo "scale=10; $l $op $r" | bc -l); val_f=$(echo "$val" | awk '{printf "%g", $1}')
      if [[ "$op" == "*" ]]; then
    ...
    if echo "$cur" | grep -qE '[0-9.]+[+\-][0-9.]+'; then
      match=$(echo "$cur" | grep -oE '[0-9.]+\s*[+\-]\s*[0-9.]+' | head -n1); clean=$(echo "$match" | tr -d ' '); val=$(echo "scale=10; $clean" | bc -l); val_f=$(echo "$val" | awk '{printf "%g", $1}')
      echo -e "${G}$(t step) $step:${END} $clean = $val_f"; cur=$(awk -v c="$cur" -v o="$match" -v n="$val_f" 'BEGIN{pos=index(c,o); print substr(c,1,pos-1) n substr(c,pos+length(o))}'); echo -e "  -> $cur"; step=$((step+1)); continue
    fi
    break
  done
...
way_equation(){
  ...
  echo -e "${Y}--- WAY: $raw ---${END}"; echo -e "${W}$(t eq_orig):${END} $left = $right"
  ...
  if (( $(echo "$a == 0" | bc -l) )); then echo -e "${R}No solution${END}"; return; fi
...
while true; do
  ...
  if [[ "$input" == *"="* ]]; then
    ...
    if [[ -n "$var" ]]; then expr="($left)-($right)"; b=$(eval_at "$expr" "$var" 0); ab=$(eval_at "$expr" "$var" 1)
      if [[ -n "$b" && -n "$ab" ]]; then a=$(echo "$ab - $b" | bc -l)
        if (( $(echo "$a == 0" | bc -l) )); then echo -e "${R}No solution${END}"; else xval=$(echo "scale=10; -($b)/($a)" | bc -l); xval_f=$(echo "$xval" | awk '{printf "%g", $1}'); echo -e "${Y}  ➜ ${W}${BOLD}$var = $xval_f${END}"; VARS[$var]=$xval_f; VARS[ans]=$xval_f; HIST+=("$input => $var=$xval_f"); LAST_EXPR="$input"; fi; continue; fi
    fi
  fi
  ...
  out="  ${Y}➜ ${W}${BOLD}$res_f${END}"; if [[ $has_per -eq 1 ]]; then frac=$(to_frac "$res"); [[ "$frac" != "$res_f" ]] && out="$out ${D}= $frac [per]${END}" || out="$out ${D}[per]${END}"; elif [[ $has_bagi -eq 1 ]]; then out="$out ${D}[bagi/divide]${END}"; fi
