...
# -------------------- TRANSLATOR --------------------
# Semua teks UI lewat fungsi t() biar gampang ganti bahasa
# LANG_SET = id atau en
t(){
  local key="$1"
  if [[ "$LANG_SET" == "en" ]]; then
...
" ;;
      ...
      infinite_solution) echo "Infinite solutions" ;;
      bagi_label) echo "[divide]" ;;
      per_label) echo "[per]" ;;
      need_bc) echo "Need 'bc' installed. Run: pkg install bc" ;;
    esac
  else
    case "$key" in
      prompt) echo "ultra> " ;;
...
${Y}${BOLD}OPERATOR:${END}
  ...
  ${G}² ³ ^ √ %${END} = pangkat/akar/persen -> 5²=25, √16=4, 50%=0.5
...
" ;;
      ...
      saved) echo "[disimpan]" ;;
      way_usage) echo "Pakai: way <rumus>  ex: way 10:2+5*2" ;;
      orig) echo "Awal" ;;
      ...
      infinite_solution) echo "Tak hingga solusi" ;;
      bagi_label) echo "[bagi]" ;;
      per_label) echo "[per]" ;;
      need_bc) echo "Butuh 'bc'. Install: pkg install bc" ;;
    esac
  fi
}
...
check_bc(){
  if ! command -v bc >/dev/null 2>&1; then
    echo -e "${R}$(t need_bc)${END}"
    exit 1
  fi
}

# -------------------- NORMALISASI EKSPRESI --------------------
# Mengubah simbol user-friendly jadi format yang dimengerti bc
# Contoh: ² -> ^2, √ -> sqrt, : -> /, × -> *
normalize_expr(){
  local e="$1"
  # superscript angka
  ...
  # : adalah bagi, ubah jadi / untuk bc
  e=$(echo "$e" | sed 's/:/\//g')
  echo "$e"
}

# -------------------- TAMBAH PERKALIAN IMPLISIT --------------------
# FIXED: tidak ada \2 invalid, semua regex aman untuk Termux sed
# 2x -> 2*x, 2( -> 2*(, )( -> )*(, )x -> )*x
add_implicit_mul(){
  local e="$1"
...
# -------------------- WAY NUMERIK --------------------
# Menampilkan langkah demi langkah, tanpa awk warning \/
way_numeric(){
  local raw="$1"
  ...
  for _ in $(seq 1 50); do
    ...
    if echo "$cur" | grep -qE '[0-9.]+\^[0-9.]+'; then
      ...
      r=$(echo "$match" | cut -d'^' -f2)
      val=$(echo "scale=10; $l ^ $r" | bc -l)
      val_f=$(echo "$val" | awk '{printf "%g", $1}')
    ...
    # kali dan bagi * /  (FIXED: pakai [*/] bukan [*\/] biar tidak warning awk)
    if echo "$cur" | grep -qE '[0-9.]+[*/][0-9.]+'; then
...
way_equation(){
  ...
  local l_const=$(eval_at "$left" "$var" 0)
  local l_one=$(eval_at "$left" "$var" 1)
  local r_const=$(eval_at "$right" "$var" 0)
  local r_one=$(eval_at "$right" "$var" 1)

  local l_coeff=$(echo "$l_one - $l_const" | bc -l)
  local r_coeff=$(echo "$r_one - $r_const" | bc -l)

  local l_coeff_f=$(echo "$l_coeff" | awk '{printf "%g", $1}')
  local l_const_f=$(echo "$l_const" | awk '{printf "%g", $1}')
  local r_coeff_f=$(echo "$r_coeff" | awk '{printf "%g", $1}')
  local r_const_f=$(echo "$r_const" | awk '{printf "%g", $1}')
...
# -------------------- PILIH BAHASA --------------------
# Request: English only prompt, bisa nomor atau nama, langsung clear setelah pilih
choose_language(){
  ...
  # support number or name
  if [[ "$lc_trim" == "2" || "$lc_trim" == "english" || "$lc_trim" == "en" || "$lc_trim" == "eng" ]]; then
    LANG_SET="en"
  elif [[ "$lc_trim" == "1" || "$lc_trim" == "indonesia" || "$lc_trim" == "id" || "$lc_trim" == "indo" ]]; then
    LANG_SET="id"
  else
    # default indonesia jika input aneh
    LANG_SET="id"
  fi
  # langsung clear pilih bahasanya
  clear
  banner
...
while true; do
  ...
  if [[ "$low" == way* ]]; then
    ...
    if [[ -z "$expr" ]]; then
      if [[ -n "$LAST_EXPR" ]]; then expr="$LAST_EXPR"
      else echo -e "${R}$(t way_usage)${END}"; continue; fi
    fi
    LAST_EXPR="$expr"
  ...
  if [[ $has_per -eq 1 ]]; then
    frac=$(to_frac "$res")
    [[ "$frac" != "$res_f" ]] && out="$out ${D}= $frac $(t per_label)${END}" || out="$out ${D}$(t per_label)${END}"
  elif [[ $has_bagi -eq 1 ]]; then
    out="$out ${D}$(t bagi_label)${END}"
  fi
  echo -e "$out"
  HIST+=("$input = $res_f")
  LAST_EXPR="$input"
