#!/bin/bash
# ULTRA CALCULATOR v4 - Bilingual + Way (cara)
# Fix sed, operator + - * : / , aljabar, no title, pilih bahasa
...
# --- PILIH BAHASA (tanpa title) ---
echo -e "${W}Pilih bahasa / Choose language:${END}"
echo "1. Indonesia"
echo "2. English"
echo -ne "> "
read -r lang_choice
...
${Y}${BOLD}COMMANDS:${END}
  ...
  ${G}way <expr>${END} -> show step-by-step solution
  ex: way 10:2+5*2
      way 2x+3=11
" ;;
        bye) echo "Bye!" ;;
        ...
        saved) echo "[saved]" ;;
        no_var) echo "variable not set. Use var=number first" ;;
        way_usage) echo "Usage: way <expression>  ex: way 10:2+5*2" ;;
        orig) echo "Original" ;;
...
${Y}${BOLD}OPERATOR:${END}
  ${G}+${END} = tambah   -> 2+3=5
  ${G}-${END} = kurang   -> 5-2=3
  ${G}*${END} = kali     -> 5*5=25 (pakai * ya, bukan x)
  ${G}:${END} = bagi     -> 10:2=5
...
${Y}${BOLD}PERINTAH:${END}
  help, vars, history, clear, exit
  ${G}way <rumus>${END} -> tampilkan cara menghitung
  contoh: way 10:2+5*2
          way 2x+3=11
" ;;
        ...
        saved) echo "[disimpan]" ;;
        no_var) echo "variabel belum ada nilai. Pakai var=angka dulu" ;;
        way_usage) echo "Pakai: way <rumus>  contoh: way 10:2+5*2" ;;
        orig) echo "Awal" ;;
...
way_numeric(){
  ...
  echo -e "${Y}--- $(t way_title 2>/dev/null || echo 'WAY') ---${END}"
  echo -e "${W}$(t orig):${END} $raw"
  if [[ "$cur_norm" != "$raw" ]]; then echo -e "${D}$(t norm): $cur_norm${END}"; fi
  if [[ "$cur_imp" != "$cur_norm" ]]; then echo -e "${D}Implicit: $cur_norm -> $cur_imp${END}"; fi
  ...
  for _ in $(seq 1 30); do
    ...
    if [[ "$cur" =~ sqrt\([^()]+\) ]]; then
      ...
      val=$(echo "scale=10; sqrt($inner)" | bc -l 2>/dev/null)
      [[ -z "$val" ]] && break
      val_f=$(echo "$val" | awk '{printf "%g", $1}')
      echo -e "${G}$(t step) $step:${END} $match = $val_f"
      cur=$(awk -v c="$cur" -v o="$match" -v n="$val_f" 'BEGIN{pos=index(c,o); if(pos) print substr(c,1,pos-1) n substr(c,pos+length(o)); else print c}')
      echo -e "  -> $cur"
    ...
    if [[ "$cur" =~ \([^()]+\) ]]; then
      ...
      val=$(echo "scale=10; $inner" | bc -l 2>/dev/null)
      [[ -z "$val" ]] && break
      val_f=$(echo "$val" | awk '{printf "%g", $1}')
      echo -e "${G}$(t step) $step:${END} $match = $val_f"
    ...
    if echo "$cur" | grep -qE '[0-9.]+\^[0-9.]+'; then
      ...
      val=$(echo "scale=10; $l ^ $r" | bc -l 2>/dev/null)
      val_f=$(echo "$val" | awk '{printf "%g", $1}')
    ...
    if echo "$cur" | grep -qE '[0-9.]+[*\/][0-9.]+'; then
      ...
      clean=$(echo "$match" | tr -d ' ')
      # pisah
      op=$(echo "$clean" | grep -oE '[*\/]')
      l=$(echo "$clean" | awk -F'[*\/]' '{print $1}')
      r=$(echo "$clean" | awk -F'[*\/]' '{print $2}')
      val=$(echo "scale=10; $l $op $r" | bc -l)
      val_f=$(echo "$val" | awk '{printf "%g", $1}')
      if [[ "$op" == "*" ]]; then
        if [[ "$LANG_SET" == "en" ]]; then echo -e "${G}$(t step) $step:${END} $l times $r = $val_f"
        else echo -e "${G}$(t step) $step:${END} $l kali $r = $val_f"; fi
      else
        if [[ "$LANG_SET" == "en" ]]; then echo -e "${G}$(t step) $step:${END} $l divided by $r = $val_f"
        else echo -e "${G}$(t step) $step:${END} $l bagi $r = $val_f"; fi
      fi
      cur=$(awk -v c="$cur" -v o="$match" -v n="$val_f" 'BEGIN{pos=index(c,o); print substr(c,1,pos-1) n substr(c,pos+length(o))}')
      echo -e "  -> $cur"
      step=$((step+1)); continue
    ...
    # + -
    # cari + / - yang bukan di awal
    if echo "$cur" | grep -qE '^.*[0-9.][+\-][0-9.]+.*$'; then
      # ambil match pertama yang valid (hindari --)
      match=$(echo "$cur" | grep -oE '[0-9.]+\s*[+\-]\s*[0-9.]+' | head -n1)
      clean=$(echo "$match" | tr -d ' ')
      val=$(echo "scale=10; $clean" | bc -l)
      val_f=$(echo "$val" | awk '{printf "%g", $1}')
  ...
  # simpan frac kalau per
  if [[ "$raw" == *"/"* ]]; then echo -e "${D}= $(to_frac $final) [per]${END}"; fi
}

way_equation(){
  local raw="$1"
  local left=$(echo "$raw" | cut -d'=' -f1)
  local right=$(echo "$raw" | cut -d'=' -f2-)
  local var=$(echo "$raw" | grep -o -E '[a-zA-Z]' | head -n1)
  [[ -z "$var" ]] && var="x"

  echo -e "${Y}--- WAY: $raw ---${END}"
  ...
  local l_coeff=$(echo "$l_one - $l_const" | bc -l)
  local r_coeff=$(echo "$r_one - $r_const" | bc -l)

  l_coeff_f=$(echo "$l_coeff" | awk '{printf "%g", $1}')
  l_const_f=$(echo "$l_const" | awk '{printf "%g", $1}')
  r_coeff_f=$(echo "$r_coeff" | awk '{printf "%g", $1}')
  r_const_f=$(echo "$r_const" | awk '{printf "%g", $1}')

  echo -e "${G}$(t step) 1:${END} ${l_coeff_f}$var + $l_const_f = ${r_coeff_f}$var + $r_const_f"

  local a=$(echo "$l_coeff - $r_coeff" | bc -l)
  local b=$(echo "$r_const - $l_const" | bc -l)
  local a_f=$(echo "$a" | awk '{printf "%g", $1}')
  local b_f=$(echo "$b" | awk '{printf "%g", $1}')

  echo -e "${G}$(t step) 2 - $(t eq_group):${END} ${a_f}$var = $b_f"
  echo -e "  ${D}$(t eq_calc): ($l_coeff_f - $r_coeff_f)$var = $r_const_f - $l_const_f${END}"

  if (( $(echo "$a == 0" | bc -l) )); then
    echo -e "${R}No solution / Tidak ada solusi${END}"; return
  fi

  local xval=$(echo "scale=10; $b / $a" | bc -l)
  local xval_f=$(echo "$xval" | awk '{printf "%g", $1}')
...
# --- LOOP UTAMA ---
clear
# sudah pilih bahasa di atas, tidak pakai title
if [[ "$LANG_SET" == "en" ]]; then echo -e "${D}Type 'help' for help, 'way <expr>' for steps${END}\n"
else echo -e "${D}Ketik 'help' untuk bantuan, 'way <rumus>' untuk cara${END}\n"; fi
...
while true; do
  ...
  # way command
  if [[ "$low" == way* ]]; then
    expr=$(echo "$raw" | cut -c4- | xargs) # ambil setelah way
    if [[ -z "$expr" ]]; then
  ...
  if [[ "$input" == *"="* ]]; then
    if [[ "$input" =~ ^[a-zA-Z_][a-zA-Z0-9_]*[[:space:]]*=[[:space:]]*[^=]+$ ]]; then
      ...
      if [[ -n "$res" ]]; then
        ...
        VARS[ans]=${VARS[$left]}
        echo -e "  ${C}$left${END} = ${BOLD}${VARS[$left]}${END} ${D}$(t saved)${END}"
        HIST+=("$input = ${VARS[$left]}"); LAST_EXPR="$input"
        continue
      fi
    fi
    ...
    if [[ -n "$var" ]]; then
      ...
      if [[ -n "$b" && -n "$ab" ]]; then
        a=$(echo "$ab - $b" | bc -l)
        if (( $(echo "$a == 0" | bc -l) )); then
          echo -e "${R}No solution${END}"
        else
          xval=$(echo "scale=10; -($b)/($a)" | bc -l)
          xval_f=$(echo "$xval" | awk '{printf "%g", $1}')
          echo -e "${Y}  ➜ ${W}${BOLD}$var = $xval_f${END}"
          VARS[$var]=$xval_f; VARS[ans]=$xval_f; HIST+=("$input => $var=$xval_f"); LAST_EXPR="$input"
        fi
        continue
      fi
    fi
  ...
  tmp=$(normalize_expr "$input"); tmp=$(add_implicit_mul "$tmp")
  ...
  if echo "$tmp2" | grep -q -E '[a-zA-Z]'; then
    ...
    if [[ "$simplified" != "$input" && -n "$simplified" ]]; then
      ...
      HIST+=("$input = $simplified"); LAST_EXPR="$input"; continue
    fi
  fi
  ...
  if [[ $has_per -eq 1 ]]; then frac=$(to_frac "$res"); [[ "$frac" != "$res_f" ]] && out="$out ${D}= $frac [per]${END}" || out="$out ${D}[per]${END}"
  elif [[ $has_bagi -eq 1 ]]; then out="$out ${D}[bagi/divide]${END}"; fi
  echo -e "$out"
  HIST+=("$input = $res_f"); LAST_EXPR="$input"
done
