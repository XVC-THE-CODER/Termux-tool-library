...
exploit_lobby() {
    ...
    if [ "$SELECTED" = "id" ]; then
        ...
        echo -e "${G}exp bypass${NC} - masuk ke discord"
        ...
        echo -e "${G}exp bypass${NC} - join discord"
    ...
    while true; do
        ...
        case "$efull" in
            ...
            exp\ bypass* )
                if [ "$SELECTED" = "id" ]; then
                    echo -e "${Y}Command ini akan masuk ke Discord, apakah Anda mau join? (y/n)${NC}"
                else
                    echo -e "${Y}This command will enter Discord, do you want to join? (y/n)${NC}"
                fi
                read -p $'\033[1;37m┗━[y/n]<$ \033[0m' yn
                yn=$(norm "$yn")
                if [ "$yn" = "y" ] || [ "$yn" = "yes" ] || [ "$yn" = "ya" ]; then
                    dlink="https://discord.gg/xzTj84EUkD"
                    if [ "$SELECTED" = "id" ]; then
                        echo -e "${G}Membuka Discord...${NC}"
                    else
                        echo -e "${G}Opening Discord...${NC}"
                    fi
                    termux-open-url "$dlink" >/dev/null 2>&1
                    am start -a android.intent.action.VIEW -d "$dlink" >/dev/null 2>&1
                else
                    if [ "$SELECTED" = "id" ]; then
