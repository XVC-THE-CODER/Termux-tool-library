...
TXT_STOP_TXT="STOP"
if [ "$_g" -le 4 ]; then TIER=1; REFRESH=60; ANIM=0.5
elif [ "$_g" -le 6 ]; then TIER=2; REFRESH=90; ANIM=0.3
else TIER=3; REFRESH=120; ANIM=0.0
fi
...
get_fps_auto(){ TEMP=$(get_temp); if [ "$TIER" -eq 1 ]; then MAX_FPS=60; elif [ "$TIER" -eq 2 ]; then MAX_FPS=90; else MAX_FPS=120; fi; if [ "$TEMP" -lt 44 ]; then echo $MAX_FPS; else echo $((MAX_FPS-5)); fi; }
has_cooler(){ if ls /sys/class/thermal/cooling_device* >/dev/null 2>&1; then return 0; elif ls /sys/class/thermal/thermal_zone* >/dev/null 2>&1; then return 0; else return 1; fi; }
if has_cooler; then COOLER_ENABLED=1; COOLER_STATUS="AUTO"; else COOLER_ENABLED=0; COOLER_STATUS="NOT SUPPORTED - DISABLED"; fi
stealth_cache_clean(){ pm trim-caches 2048M >/dev/null 2>&1; for p in /sdcard/Android/data/*/cache; do rm -rf $p/* >/dev/null 2>&1; done; }
ultra_anti_ads(){ safe_set global private_dns_mode hostname; safe_set global private_dns_specifier $(echo ZG5zLmFkZ3VhcmQuY29t | base64 -d); }
ultra_anti_lag(){ safe_set global cached_apps_freezer enabled; }
boost_cpu_gpu(){ safe_set global sem_enhanced_cpu_responsiveness 1; safe_set global debug.hwui.renderer skiagl; safe_set global window_animation_scale $ANIM; }
slippery_logic(){ FPS=$2; if [ "$FPS" -ge 55 ]; then SLOP=4; TO=150; else SLOP=8; TO=190; fi; safe_set system view_configuration_touch_slop $SLOP; safe_set secure long_press_timeout $TO; }
game_loader_boost(){ for pkg in $(pm list packages -3 2>/dev/null | cut -d: -f2 | grep -i -E "mobile|legend|pubg|minecraft|mlbb" | head -3); do cmd package compile -m speed-profile -f $pkg >/dev/null 2>&1 & done; }
...
draw_box(){
  ...
  if [ "$COOLER_ENABLED" -eq 1 ]; then echo -e "  ${G}[✓]${NC} ${W}cooler : $COOLER_INFO${NC}"; else echo -e "  ${R}[x]${NC} ${W}cooler : $COOLER_STATUS${NC}"; fi
  echo -e "${C}┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┫${NC}"
...
while true; do
  ...
  notify_start "anti lag"
  ultra_anti_lag
  notify_clear
  notify_start "block ads"
  ultra_anti_ads
  notify_clear
  notify_start "cpu gpu"
  boost_cpu_gpu
  notify_clear
  notify_start "slippery sensitivity"
  slippery_logic $TEMP $FPS
  notify_clear
  notify_start "game loading"
  game_loader_boost
  notify_clear
  notify_start "map gen"
  map_gen_boost
  notify_clear
  if [ "$COOLER_ENABLED" -eq 1 ]; then
    notify_start "$COOLER_INFO"
    sleep 0.3
    notify_clear
  fi
  if [ $((CYCLE % 5)) -eq 0 ]; then
    notify_start "cache clean"
    stealth_cache_clean
    notify_clear
  fi
  echo -e "${G}[$(date +%T)] ALL DONE - STABLE${NC}"
  echo -e "${W}>> $TXT_TEKAN <<${NC}"
  if read -t 1.5; then clear; echo -e "${G}✔ $TXT_STOP_TXT${NC}"; safe_set global private_dns_mode opportunistic; if command -v termux-notification-remove >/dev/null 2>&1; then termux-notification-remove tool_up >/dev/null 2>&1; fi; exit 0; fi
done
